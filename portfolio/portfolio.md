...
slug: saunau
name: 사우나우
tagline: 후기 없는 동네 목욕탕까지 찾아주는 지도
year: "2026"
logo_bg: "#22201E"
logo_file: ./logo.svg
...
전국 목욕탕·찜질방·온천·세신샵 6천여 곳을 지도와 목록으로 여는 웹앱이에요. 위치를 켜면 주변이 거리순으로 깔리고, 상단 칩 하나로 "24시 되는 찜질방"이나 "세신샵"만 남길 수 있습니다. 화면을 옮기면 그 지역에서 다시 검색해요.

한 곳을 열면 사우나실 온도, 영업시간, 요금표, 사진, 블로그 후기, 위치가 한 화면에 들어옵니다. 원래는 블로그를 열 개쯤 열어 놓고 짜맞춰야 알던 것들이에요. 사우나우는 그 짜맞추는 일을 미리 해둔 상태로 보여줍니다. 마음에 든 곳은 찜해 두고, 다녀온 곳은 메모로 남겨요. 로그인해 두면 기기를 바꿔도 따라옵니다.

모바일 웹앱이에요. 데스크톱에서는 화면 중앙에 430px 프레임으로 고정되고, 지도만 풀블리드로 열립니다.

## 기획

**계기** 내 주변 목욕탕이랑, 검색으로는 안 나오는 숨은 목욕탕을 후기까지 같이 보고 싶었어요. 그런 게 한국에는 없더라고요
**벤치마크** 일본 사우나이키타이 — https://sauna-ikitai.com/ . 사우나를 온도·시설 축으로 정리해 두는 방식을 참고했어요
**벤치마크** 인스타 고독한사우너 — https://www.instagram.com/godoghan_sauner/ . 직접 발로 뛰어 욕탕 사진을 찍고 장단점을 적어 올리는데, 구독자로서 그게 제일 쓸모 있었어요. 그 밀도의 정보가 한곳에 모이면 좋겠다는 게 출발이었습니다
...
**문제** 네이버 지도에 "목욕탕"이라고 쳐도 안 나오는 곳이 있어요. 호텔·리조트 부속이나 복지시설 안에 있으면 상호에 그 낱말이 없어서 검색에 안 걸립니다
**사용자** 사우나를 즐겨 다니는 2030. 주말이나 퇴근길에 "지금 갈 만한 곳"을 3탭 안에 찾고 싶어해요
**가설** 검색 기반이 아니라 인허가 전수 기반으로 깔면, 검색어에 안 걸리던 숨은 목욕탕이 저절로 딸려 온다
**제약** 온도는 결국 사람이 채우는 값이라 콜드 데이터에는 거의 없어요. 크롤로 채워지는 축을 기본으로 깔고 온도는 제보가 쌓이면 켜지는 자리로 뒀습니다
**제약** 혼자 만들고 혼자 운영해요. 사람이 붙어야 굴러가는 기능은 처음부터 후보에서 뺐습니다
**넣지 않은 것** 로그인. 디바이스 식별로 충분하다고 봤어요
**넣지 않은 것** 자유 텍스트 리뷰·사진 업로드. 모더레이션을 감당할 사람이 없어요
**넣지 않은 것** 예약·결제. 매장 온보딩이 필요한데 그건 수요가 확인된 다음 일이에요
...
**설명** 카드에 들어간 그림은 용어마다 직접 그린 전용 SVG 라, 지도 칩이나 필터에서도 같은 목욕탕·찜질방 그림이 나와요

### 지도

**파일** ./screens/02-map.png
**설명** 위치 권한을 안 주면 전국이 들어오게 맞추고, 줌 9 미만에서는 시·도 집계 버블로 묶습니다 — 버블 좌표는 소속 매장 평균이 아니라 도청 소재지로 고정했어요

### 매장 상세

**파일** ./screens/03-detail.png
**설명** 사진은 네이버 업체제공분만 골라 우리 Storage 에 재호스팅한 것이고, 요금표는 플레이스에서, 후기 5건은 블로그 링크아웃으로 붙습니다

### 읽을거리

**파일** ./screens/04-feed.png
**설명** 커버 그림은 앱 일러스트와 결을 따로 잡은 만화체 SVG 로 글마다 하나씩 그렸어요
...
**갈라짐** 가볼 만하다 → 찜 / 정보가 부족하다 → 블로그 후기 원문으로 나감
...
- [x] 내 주변 갈 만한 곳을 3탭 안에 찾는다
- [x] 공공데이터로 전국을 깔아서 검색에 안 걸리던 곳까지 나온다
- [x] 큐레이션이랑 읽을거리로 안 가본 곳을 둘러본다
- [x] 매장 상세가 검색에 잡히게 만든다
...
### 4. 안 하기로 한 걸 두 번 뒤집고, 하기로 한 걸 하나 접었어요
...
**붙인 것** Supabase Auth, sharp, Google Cloud Vision
...
**적재** `pnpm crawl:naver-sesin` 은 1인 세신샵을, `pnpm crawl:naver-enzyme` 은 효소찜질을 별도 플래그로 적재한다. 목욕장업 인허가에는 거의 없는 업종이라 이 경로가 유일한 출처다
...
**주의** 무료 한도가 SKU 별 월 단위다 — Text Search Pro 월 5,000회, Place Photos 월 1,000회. 필드마스크에 든 최고 등급 필드가 요청 전체의 요금을 정한다. `websiteUri` 는 Enterprise 라 하나만 넣어도 무료분이 1,000회로 줄어든다. 업주가 올린 사진인지 구분할 방법이 없는 것도 중단 이유다
...
영업 상태로 거르고, 상호·업태·주소에서 유형을 추론하고, 좌표가 빠진 행만 지오코딩해요. 분류 규칙은 마이그레이션 백필과 코드가 같은 신호를 쓰게 맞춰 둡니다 — 한쪽만 고치면 다음 동기화에서 되돌아가요.
...
- 온도 제보가 0건이다. 제보 동선을 다시 설계하지 않으면 표시 임계치를 영영 못 넘는다.
